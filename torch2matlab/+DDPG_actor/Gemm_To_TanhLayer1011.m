classdef Gemm_To_TanhLayer1011 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = Gemm_To_TanhLayer1011(name, onnxParams)
            this.Name = name;
            this.NumInputs = 2;
            this.OutputNames = {'x16'};
        ...
        function [x16] = predict(this, input, inputNumDims)
            if isdlarray(input)
            ...
            [x16, x16NumDims] = Gemm_To_TanhFcn(input, inputNumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {['as-is'], ['as-is']}, ...
                'OutputDataPermutation', {['as-is'], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x16}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'Gemm_To_TanhLayer1011');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'Gemm_To_TanhLayer1011'));
            end
            x16 = dlarray(single(x16), repmat('U', 1, max(2, x16NumDims)));
            if ~coder.target('MATLAB')
                x16 = extractdata(x16);
            end
        end
        
        function [x16] = forward(this, input, inputNumDims)
            if isdlarray(input)
                input = stripdims(input);
            end
            inputNumDims = numel(inputNumDims);
            ...
            onnxParams.Learnables.fc4_bias = this.fc4_bias;
            onnxParams.Learnables.fc4_weight = this.fc4_weight;
            [x16, x16NumDims] = Gemm_To_TanhFcn(input, inputNumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {['as-is'], ['as-is']}, ...
                'OutputDataPermutation', {['as-is'], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x16}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'Gemm_To_TanhLayer1011');
...
function [x16, x16NumDims, state] = Gemm_To_TanhFcn(input, inputNumDims, params, varargin)
...
%
% [X16] = Gemm_To_TanhFcn(INPUT, PARAMS)
...
%
% [X16, STATE] = Gemm_To_TanhFcn(INPUT, PARAMS)
%			- Additionally returns state variables in STATE. When training,
%			use this form and set TRAINING to true.
%
% [__] = Gemm_To_TanhFcn(INPUT, PARAMS, 'NAME1', VAL1, 'NAME2', VAL2, ...)
...
[x16, x16NumDims, state] = Gemm_To_TanhGraph1000(input, NumDims.input, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x16] = postprocessOutput(x16, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x16, x16NumDims1010, state] = Gemm_To_TanhGraph1000(input, inputNumDims1009, Vars, NumDims, Training, state)
% Function implementing the graph 'Gemm_To_TanhGraph1000'
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_fc1_Gemm_output_0] = prepareGemmArgs(Vars.input, Vars.fc1_weight, Vars.fc1_bias, Vars.Gemmalpha1001, Vars.Gemmbeta1002, 0, 1, NumDims.fc1_bias);
...
% Tanh:
Vars.x16 = tanh(Vars.x_fc4_Gemm_output_0);
NumDims.x16 = NumDims.x_fc4_Gemm_output_0;
