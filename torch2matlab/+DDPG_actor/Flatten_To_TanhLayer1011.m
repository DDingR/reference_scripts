classdef Flatten_To_TanhLayer1011 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = Flatten_To_TanhLayer1011(name, onnxParams)
            this.Name = name;
            this.NumInputs = 2;
            this.OutputNames = {'x17'};
        ...
        function [x17] = predict(this, input, inputNumDims)
            if isdlarray(input)
            ...
            [x17, x17NumDims] = Flatten_To_TanhFcn(input, inputNumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {['as-is'], ['as-is']}, ...
                'OutputDataPermutation', {['as-is'], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x17}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'Flatten_To_TanhLayer1011');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'Flatten_To_TanhLayer1011'));
            end
            x17 = dlarray(single(x17), repmat('U', 1, max(2, x17NumDims)));
            if ~coder.target('MATLAB')
                x17 = extractdata(x17);
            end
        end
        
        function [x17] = forward(this, input, inputNumDims)
            if isdlarray(input)
                input = stripdims(input);
            end
            inputNumDims = numel(inputNumDims);
            ...
            onnxParams.Learnables.fc4_bias = this.fc4_bias;
            onnxParams.Learnables.fc4_weight = this.fc4_weight;
            [x17, x17NumDims] = Flatten_To_TanhFcn(input, inputNumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {['as-is'], ['as-is']}, ...
                'OutputDataPermutation', {['as-is'], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {x17}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'Flatten_To_TanhLayer1011');
...
function [x17, x17NumDims, state] = Flatten_To_TanhFcn(input, inputNumDims, params, varargin)
%FLATTEN_TO_TANHFCN Function implementing an imported ONNX network.
%
...
%
% [X17] = Flatten_To_TanhFcn(INPUT, PARAMS)
%			- Evaluates the imported ONNX network FLATTEN_TO_TANHFCN with input(s)
%			INPUT and the imported network parameters in PARAMS. Returns
%			network output(s) in X17.
%
% [X17, STATE] = Flatten_To_TanhFcn(INPUT, PARAMS)
%			- Additionally returns state variables in STATE. When training,
%			use this form and set TRAINING to true.
%
% [__] = Flatten_To_TanhFcn(INPUT, PARAMS, 'NAME1', VAL1, 'NAME2', VAL2, ...)
...
[x17, x17NumDims, state] = Flatten_To_TanhGraph1000(input, NumDims.input, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x17] = postprocessOutput(x17, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x17, x17NumDims1010, state] = Flatten_To_TanhGraph1000(input, inputNumDims1009, Vars, NumDims, Training, state)
% Function implementing the graph 'Flatten_To_TanhGraph1000'
...
% Flatten:
[dim1, dim2, NumDims.x_Flatten_Flatten_ou] = prepareFlattenArgs(Vars.input, 1, NumDims.input);
Vars.x_Flatten_Flatten_ou = reshape(Vars.input, dim1, dim2);

% Gemm:
[A, B, C, alpha, beta, NumDims.x_fc1_Gemm_output_0] = prepareGemmArgs(Vars.x_Flatten_Flatten_ou, Vars.fc1_weight, Vars.fc1_bias, Vars.Gemmalpha1001, Vars.Gemmbeta1002, 0, 1, NumDims.fc1_bias);
Vars.x_fc1_Gemm_output_0 = alpha*B*A + beta*C;

% Relu:
Vars.x_relu1_Relu_output_ = relu(Vars.x_fc1_Gemm_output_0);
NumDims.x_relu1_Relu_output_ = NumDims.x_fc1_Gemm_output_0;

% Gemm:
[A, B, C, alpha, beta, NumDims.x_fc2_Gemm_output_0] = prepareGemmArgs(Vars.x_relu1_Relu_output_, Vars.fc2_weight, Vars.fc2_bias, Vars.Gemmalpha1003, Vars.Gemmbeta1004, 0, 1, NumDims.fc2_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_fc3_Gemm_output_0] = prepareGemmArgs(Vars.x_relu2_Relu_output_, Vars.fc3_weight, Vars.fc3_bias, Vars.Gemmalpha1005, Vars.Gemmbeta1006, 0, 1, NumDims.fc3_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_fc4_Gemm_output_0] = prepareGemmArgs(Vars.x_relu3_Relu_output_, Vars.fc4_weight, Vars.fc4_bias, Vars.Gemmalpha1007, Vars.Gemmbeta1008, 0, 1, NumDims.fc4_bias);
Vars.x_fc4_Gemm_output_0 = alpha*B*A + beta*C;

% Tanh:
Vars.x17 = tanh(Vars.x_fc4_Gemm_output_0);
NumDims.x17 = NumDims.x_fc4_Gemm_output_0;

% Set graph output arguments from Vars and NumDims:
x17 = Vars.x17;
x17NumDims1010 = NumDims.x17;
% Set output state from Vars:
state = updateStruct(state, Vars);
end

function [inputDataPerms, outputDataPerms, Training] = parseInputs(input, numDataOutputs, params, varargin)
% Function to validate inputs to Flatten_To_TanhFcn:
p = inputParser;
...
x17 = permuteOutputVar(x17, outputDataPerms{1}, 2);
end
